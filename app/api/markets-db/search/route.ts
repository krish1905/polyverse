// Search markets from our DB instead of Polymarket API
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Search in Supabase using ilike (case-insensitive)
    const { data, error } = await supabase
      .from('markets')
      .select('*')
      .or(`question.ilike.%${query}%,description.ilike.%${query}%,category.ilike.%${query}%`)
      .eq('active', true)
    ...
    console.log(`[DB Search] Found ${data?.length || 0} matches`);
